...
int LightSSS::do_fork() {
  ...
  waitProcess = 1;
  forkshm.shwait();
  fprintf(stderr, "[FORK_INFO pid(%d)] fork child process %d waked up\n",
          getpid(), getppid());
  // checkpoint process wakes up
  // start wave dumping
  if (forkshm.info->oldest != getpid()) {
    FORK_PRINTF("Error, non-oldest process should not live. Parent Process "
                "should kill the process manually.\n")
    return FORK_ERROR;
  }
  return FORK_CHILD;
}
...
int LightSSS::wakeup_child(uint64_t cycles) {
  ...
  fprintf(stderr, "[FORK_INFO pid(%d)] wait for child process %d\n", getpid(),
          pidSlot.back());
